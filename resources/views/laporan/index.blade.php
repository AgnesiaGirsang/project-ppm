...
<div class="card">
  ...
  <div class="sub" style="margin-bottom:18px; font-size:12.5px; color:var(--ink-500);">
    Daftar pengajuan yang sudah berada di tahap {{ $judulHalaman }}{{ $tipe === 'kemajuan' ? ' (khusus jalur Simlitabkes)' : '' }}.
  </div>

  <table>
    <thead><tr><th>Kode</th><th>Judul</th><th>Skema</th><th>Status Laporan</th><th>Aksi</th></tr></thead>
    <tbody>
      @forelse ($daftar as $p)
        @php
        ...
        <tr>
          ...
          <td><a href="{{ route('laporan.form', [$tipe, $p]) }}" class="btn btn-outline" style="padding:6px 14px; font-size:12px;">{{ $laporan ? 'Lihat / Unggah Ulang' : 'Unggah' }}</a></td>
        </tr>
      @empty
        <tr><td colspan="5" style="text-align:center; color:var(--ink-500); padding:24px;">Belum ada pengajuan yang berada di tahap ini.</td></tr>
      @endforelse
    </tbody>
  </table>
</div>
