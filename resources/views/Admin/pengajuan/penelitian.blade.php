...
@section('content')
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6">
        ...
        <form method="GET" action="{{ route('admin.penelitian') }}"
            class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-7 gap-3 mb-6">
            ...
            <table class="w-full text-left border-collapse text-xs min-w-[700px]" id="tabelPenelitian">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($pengajuans as $index => $p)
                        @php
                            $badgeColor = match (strtolower($p->status)) {
                                'disetujui' => 'bg-emerald-100 text-emerald-800',
                                'revisi', 'direvisi' => 'bg-rose-100 text-rose-800',
                                default => 'bg-amber-100 text-amber-800',
                            };
                            $statusText = match (strtolower($p->status)) {
                                'disetujui' => 'Disetujui',
                                'revisi' => 'Direvisi',
                                'proses' => 'Dalam Proses',
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="py-4 px-3">
                                <span
                                    class="px-3 py-1 rounded-full text-[10px] font-bold {{ $p->jalur == 'Simlitabkes' ? 'bg-emerald-100 text-emerald-800' : 'bg-purple-100 text-purple-800' }}">
                                    {{ $p->jalur ?? '-' }}
                                </span>
                            </td>
                            <td class="py-4 px-3">
                            ...
                            <td class="py-4 px-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <a href="{{ route('admin.pengajuan.download', $p->id) }}"
                                        class="w-8 h-8 inline-flex items-center justify-center rounded-lg bg-slate-100 text-slate-700 hover:bg-emerald-800 hover:text-white transition shadow-sm"
