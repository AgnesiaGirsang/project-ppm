...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6">
    ...
    <!-- Filter & Search Bar (Menggunakan Form GET) -->
    <form method="GET" action="{{ route('admin.pengabdian') }}" class="grid grid-cols-1 md:grid-cols-7 gap-3 mb-6">
        ...
        <select name="dosen" class="border border-slate-200 rounded-xl px-3 py-2.5 text-xs bg-white text-slate-700 font-semibold focus:outline-none focus:border-emerald-700">
            ...
            @foreach($listDosen ?? [] as $dosen)
                <option value="{{ $dosen->nama }}" {{ request('dosen') == $dosen->nama ? 'selected' : '' }}>{{ $dosen->nama }}</option>
            @endforeach
        </select>
        ...
        <select name="skema" class="border border-slate-200 rounded-xl px-3 py-2.5 text-xs bg-white text-slate-700 font-semibold focus:outline-none focus:border-emerald-700">
            ...
            @foreach($listSkema ?? [] as $skema)
                <option value="{{ $skema->id }}" {{ request('skema') == $skema->id ? 'selected' : '' }}>{{ $skema->nama_skema }}</option>
            @endforeach
        </select>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.pengabdian') }}" class="flex-1 bg-slate-100 text-slate-700 rounded-xl text-xs font-bold py-2.5 hover:bg-slate-200 transition text-center flex items-center justify-center">Reset</a>
        </div>
    </form>
        ...
        <table class="w-full text-left border-collapse text-xs min-w-[800px]">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="py-4 px-3 font-medium text-slate-700">{{ $item->skema->nama_skema ?? '-' }}</td>
                    <td class="py-4 px-3 font-medium text-slate-600">{{ $item->lokasi ?? '-' }}</td>
                    ...
                    <td class="py-4 px-3">
                        @php
                            $badgeColor = match($item->status) {
                                ...
                                'Direvisi' => 'bg-rose-100 text-rose-800',
                                default => 'bg-amber-100 text-amber-800',
                            };
                        @endphp
                        <span class="px-3 py-1 rounded-full text-[10px] font-bold {{ $badgeColor }} shadow-sm">
                            {{ $item->status }}
                        </span>
                    </td>
                    <td class="py-4 px-3 text-center">
                        <a href="#" class="w-8 h-8 inline-flex items-center justify-center rounded-lg bg-slate-100 text-slate-700 hover:bg-emerald-800 hover:text-white transition shadow-sm">
                ...
                @empty
                <tr>
                    <td colspan="9" class="py-6 text-center text-slate-400 font-medium">Belum ada data pengajuan pengabdian yang tersedia.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
