    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm relative flex flex-col justify-between">
            <div class="flex justify-between items-start">
                ...
                <div class="w-8 h-8 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center text-xs"><i class="fa-solid fa-file-lines"></i></div>
            </div>
            <div class="mt-4"><h3 class="text-2xl font-extrabold text-slate-900">{{ $totalSemua ?? 0 }}</h3></div>
        </div>
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm relative flex flex-col justify-between">
            <div class="flex justify-between items-start">
                <span class="text-[10px] font-extrabold tracking-wider text-amber-600 uppercase">Dalam Proses</span>
                <div class="w-8 h-8 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center text-xs"><i class="fa-solid fa-clock"></i></div>
            </div>
            <div class="mt-4"><h3 class="text-2xl font-extrabold text-slate-900">{{ $totalProses ?? 0 }}</h3></div>
        </div>
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm relative flex flex-col justify-between">
            <div class="flex justify-between items-start">
                <span class="text-[10px] font-extrabold tracking-wider text-emerald-600 uppercase">Disetujui</span>
                <div class="w-8 h-8 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center text-xs"><i class="fa-solid fa-check"></i></div>
            </div>
            <div class="mt-4"><h3 class="text-2xl font-extrabold text-slate-900">{{ $totalDisetujui ?? 0 }}</h3></div>
        </div>
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm relative flex flex-col justify-between">
            <div class="flex justify-between items-start">
                <span class="text-[10px] font-extrabold tracking-wider text-rose-600 uppercase">Perlu Revisi</span>
                <div class="w-8 h-8 rounded-xl bg-rose-50 text-rose-600 flex items-center justify-center text-xs"><i class="fa-solid fa-rotate-left"></i></div>
            </div>
            <div class="mt-4"><h3 class="text-2xl font-extrabold text-slate-900">{{ $totalRevisi ?? 0 }}</h3></div>
        </div>
    </div>

    <!-- Filter & Search Bar -->
    <form method="GET" action="{{ route('admin.semua-pengajuan') }}" class="grid grid-cols-1 md:grid-cols-7 gap-3 mb-6">
        ...
        <select name="jenis" class="border border-slate-200 rounded-xl px-3 py-2.5 text-xs bg-white text-slate-700 font-semibold focus:outline-none focus:border-emerald-700">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.semua-pengajuan') }}" class="flex-1 bg-slate-100 text-slate-700 rounded-xl text-xs font-bold py-2.5 hover:bg-slate-200 transition flex items-center justify-center">Reset</a>
        </div>
    </form>
        ...
        <table class="w-full text-left border-collapse text-xs min-w-[800px]" id="tabelSemuaPengajuan">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="py-4 px-3">
                        <span class="px-3 py-1 rounded-full text-[10px] font-bold {{ $item->jalur == 'Simlitabkes' ? 'bg-emerald-100 text-emerald-800' : 'bg-purple-100 text-purple-800' }}">
                            {{ $item->jalur ?? '-' }}
                        </span>
                    </td>
                    <td class="py-4 px-3 font-medium text-slate-600">{{ $item->created_at ? $item->created_at->format('d/m/Y') : '-' }}</td>
                    <td class="py-4 px-3">
                        @php
                        ...
                        @endphp
                        <span class="px-3 py-1 rounded-full text-[10px] font-bold {{ $badgeColor }} shadow-sm capitalize">
                            {{ $item->status }}
                        </span>
                    </td>
                    <td class="py-4 px-3 text-center">
                        <!-- Tombol aksi ikon mata diarahkan langsung ke file dokumen -->
<a href="{{ route('admin.pengajuan.dokumen', $item->id) }}" target="_blank" class="w-8 h-8 inline-flex items-center justify-center rounded-lg bg-slate-100 text-slate-700 hover:bg-emerald-800 hover:text-white transition shadow-sm" title="Lihat Dokumen Proposal">
    ...
    <!-- Pagination Footer -->
    <div class="flex flex-col sm:flex-row justify-between items-center pt-6 border-t border-slate-200 mt-4 text-xs text-slate-500 font-semibold gap-3">
