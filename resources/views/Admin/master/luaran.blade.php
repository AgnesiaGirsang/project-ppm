...
<div class="container-fluid px-6 py-8">
    ...
    <div class="bg-white rounded-2xl custom-card-shadow border border-gray-100 overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm text-gray-600 divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-emerald-50/30 transition duration-150 group">
                        ...
                        <td class="py-4 px-6">
                            @if($item->wajib)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-rose-50 text-rose-600 border border-rose-100">
                                    Wajib
                                </span>
                            @else
                        ...
                        <td class="py-4 px-6 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="bukaModalEdit('{{ $item->id }}', '{{ addslashes($item->nama) }}', '{{ addslashes($item->opsi) }}', '{{ $item->wajib ? 1 : 0 }}', '{{ $item->jenis }}')" 
                                ...
                                <form id="delete-form-{{ $item->id }}" action="{{ route('admin.master.luaran.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="flex flex-col md:flex-row justify-between items-center px-6 py-4 bg-gray-50/50 border-t border-gray-100 text-xs text-gray-500 gap-4">
            <div>
                Menampilkan <span class="font-semibold text-gray-700">{{ $luarans->firstItem() ?? 0 }}</span> sampai <span class="font-semibold text-gray-700">{{ $luarans->lastItem() ?? 0 }}</span> dari <span class="font-semibold text-gray-700">{{ $luarans->total() }}</span> data
            </div>
...
<!-- MODAL TAMBAH -->
<div id="modal-tambah" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100">
        <div class="bg-emerald-900 px-6 py-4 flex justify-between items-center text-white">
            <h3 class="text-sm font-bold flex items-center gap-2"><i class="fa-solid fa-plus-circle"></i> Tambah Luaran Baru</h3>
            <button onclick="document.getElementById('modal-tambah').classList.add('hidden')" class="text-emerald-200 hover:text-white"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
        <form action="{{ route('admin.master.luaran.store') }}" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <select name="jenis" class="w-full border border-gray-200 rounded-xl p-3 text-xs bg-white outline-none focus:ring-2 focus:ring-emerald-500" required>
            ...
            <div>
                ...
                <input type="text" name="nama" placeholder="Contoh: Artikel ilmiah dimuat di jurnal" class="w-full border border-gray-200 rounded-xl p-3 text-xs outline-none focus:ring-2 focus:ring-emerald-500" required>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Opsi / Kategori (Opsional)</label>
                <input type="text" name="opsi" placeholder="Contoh: Internasional, Nasional Terakreditasi" class="w-full border border-gray-200 rounded-xl p-3 text-xs outline-none focus:ring-2 focus:ring-emerald-500">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Status Wajib / Tambahan</label>
                <select name="wajib" class="w-full border border-gray-200 rounded-xl p-3 text-xs bg-white outline-none focus:ring-2 focus:ring-emerald-500" required>
                    <option value="1">Wajib</option>
                    <option value="0">Tambahan</option>
                </select>
            </div>
            <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button type="button" onclick="document.getElementById('modal-tambah').classList.add('hidden')" class="px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-xs font-bold">Batal</button>
                <button type="submit" class="px-5 py-2.5 btn-gradient-animate text-white rounded-xl text-xs font-bold">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<!-- MODAL EDIT -->
<div id="modal-edit" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100">
        <div class="bg-emerald-900 px-6 py-4 flex justify-between items-center text-white">
            <h3 class="text-sm font-bold flex items-center gap-2"><i class="fa-solid fa-pen-to-square"></i> Edit Luaran</h3>
            <button onclick="document.getElementById('modal-edit').classList.add('hidden')" class="text-emerald-200 hover:text-white"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
        <form id="form-edit" method="POST" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Kategori Utama</label>
                <select id="edit-jenis" name="jenis" class="w-full border border-gray-200 rounded-xl p-3 text-xs bg-white outline-none focus:ring-2 focus:ring-emerald-500" required>
                    <option value="penelitian">Penelitian</option>
                    <option value="pengabdian">Pengabdian</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Jenis Luaran</label>
                <input type="text" id="edit-nama" name="nama" class="w-full border border-gray-200 rounded-xl p-3 text-xs outline-none focus:ring-2 focus:ring-emerald-500" required>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Opsi / Kategori (Opsional)</label>
                <input type="text" id="edit-opsi" name="opsi" class="w-full border border-gray-200 rounded-xl p-3 text-xs outline-none focus:ring-2 focus:ring-emerald-500">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Status Wajib / Tambahan</label>
                <select id="edit-wajib" name="wajib" class="w-full border border-gray-200 rounded-xl p-3 text-xs bg-white outline-none focus:ring-2 focus:ring-emerald-500" required>
                    <option value="1">Wajib</option>
                    <option value="0">Tambahan</option>
                </select>
            </div>
            <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button type="button" onclick="document.getElementById('modal-edit').classList.add('hidden')" class="px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-xs font-bold">Batal</button>
                <button type="submit" class="px-5 py-2.5 btn-gradient-animate text-white rounded-xl text-xs font-bold">Perbarui</button>
            </div>
        </form>
    </div>
</div>
