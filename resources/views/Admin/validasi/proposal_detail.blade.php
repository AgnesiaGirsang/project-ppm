...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <!-- KOLOM KIRI: Informasi Pengajuan, Tim Peneliti, Luaran & Dokumen Proposal -->
        <div class="lg:col-span-7 space-y-5">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 space-y-4 text-xs">
                ...
                <div class="grid grid-cols-1 gap-3.5">
                    <div>
                        <span class="text-slate-400 font-semibold block text-[11px] uppercase tracking-wider mb-0.5">Kode
                            Pengajuan</span>
                        <span class="font-bold text-slate-900 text-xs">{{ $selected->kode ?? '-' }}</span>
                    </div>
                    <div>
                        <span
                    ...
                    <div>
                        ...
                        <span class="font-bold text-slate-900 text-xs block">{{ $selected->pegawai->nama ?? '-' }}</span>
                    </div>
                    <div>
                        <span
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <span
                                ...
                                Kegiatan</span>
                            <span
                                class="font-bold text-slate-900 text-xs block">{{ ucfirst($selected->jenis ?? '-') }}</span>
                        </div>
                        <div>
                            <span
                                class="text-slate-400 font-semibold block text-[11px] uppercase tracking-wider mb-0.5">Jalur</span>
                            <span
                                class="font-bold text-slate-900 text-xs block">{{ ucfirst($selected->jalur ?? '-') }}</span>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <span
                                ...
                                Anggaran</span>
                            <span
                                class="font-bold text-slate-900 text-xs block">{{ $selected->tahun_anggaran ?? '-' }}</span>
                        </div>
                        <div>
                            <span
                                class="text-slate-400 font-semibold block text-[11px] uppercase tracking-wider mb-0.5">Total
                                Biaya</span>
                            <span class="font-bold text-slate-900 text-xs block">Rp
                                {{ number_format($selected->total_biaya ?? 0, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 space-y-3 text-xs">
                <h3 class="font-extrabold text-slate-900 text-sm uppercase tracking-wide border-b border-slate-100 pb-3">Tim
                    Peneliti</h3>
                <div class="space-y-2.5">
                    @forelse ($selected->tim as $anggota)
                        <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                            <div
                                class="w-8 h-8 rounded-full bg-emerald-50 text-emerald-700 flex items-center justify-center font-bold">
                                <i class="fa-solid fa-user text-xs"></i>
                            </div>
                            <div>
                                <span class="font-bold text-slate-800 block text-xs">
                                    {{ $anggota->pegawai->nama ?? ($anggota->nama_luar ?? '-') }}
                                    @if (!$anggota->pegawai_id && $anggota->instansi_luar)
                                        <span
                                            class="text-[10px] text-slate-400 font-normal">({{ $anggota->instansi_luar }})</span>
                                    @endif
                                </span>
                                <span class="text-[10px] text-slate-400 uppercase">{{ $anggota->peran }}</span>
                            </div>
                        </div>
                    @empty
                        <p class="text-slate-400 text-xs italic">Belum ada data tim.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 space-y-3 text-xs">
                <h3 class="font-extrabold text-slate-900 text-sm uppercase tracking-wide border-b border-slate-100 pb-3">
                    Luaran yang Direncanakan</h3>
                <div class="space-y-2.5">
                    @forelse ($selected->luaran as $l)
                        <div
                            class="flex items-center justify-between gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                            <div>
                                <span
                                    class="font-bold text-slate-800 block text-xs">{{ $l->luaranMaster->nama ?? '-' }}</span>
                                @if ($l->opsi_dipilih)
                                    <span class="text-[10px] text-slate-400">Kategori: {{ $l->opsi_dipilih }}</span>
                                @endif
                            </div>
                            <span
                                class="inline-block px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase {{ $l->luaranMaster->wajib ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600' }}">
                                {{ $l->luaranMaster->wajib ? 'Wajib' : 'Tambahan' }}
                            </span>
                        </div>
                    @empty
                        <p class="text-slate-400 text-xs italic">Belum ada luaran yang direncanakan.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 space-y-3 text-xs">
                ...
                <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <span
                                class="font-bold text-slate-800 block text-xs">{{ $selected->proposal_nama_asli ?? basename($selected->proposal_path ?? '-') }}</span>
                            <span
                                class="text-[10px] text-slate-400">{{ $selected->proposal_size ? number_format($selected->proposal_size / 1024, 1) . ' KB' : '-' }}</span>
                        </div>
                    </div>
                    @if ($selected->proposal_path)
                        <div class="flex items-center gap-3 text-xs font-bold">
                            <a href="{{ asset('storage/' . $selected->proposal_path) }}" target="_blank"
                                class="text-emerald-700 hover:underline">Preview</a>
                            <a href="{{ asset('storage/' . $selected->proposal_path) }}"
                                download="{{ $selected->proposal_nama_asli }}"
                    ...
                    <div class="space-y-3">
                        <label class="flex items-start gap-3 cursor-pointer">
                            ...
                            <div>
                                ...
                                <span class="text-[11px] text-slate-400">Proposal dinyatakan valid dan dapat
                                    dilanjutkan</span>
                            </div>
                        </label>
                        <label class="flex items-start gap-3 cursor-pointer">
