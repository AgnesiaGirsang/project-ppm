...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 space-y-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 pb-4">
        <div>
            ...
            <p class="text-[11px] text-slate-400 font-medium">Pilih berkas yang ingin divalidasi melalui tombol aksi di tabel bawah.</p>
        </div>
        <div class="bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold px-3.5 py-1.5 rounded-xl shadow-2xs">
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700 font-medium">
                ...
                <tr class="hover:bg-slate-50/80 transition align-middle">
                    ...
                    <td class="p-3 space-y-1.5 max-w-sm">
                        ...
                        <div>
                            <span class="inline-block bg-emerald-50 text-emerald-700 border border-emerald-200/80 px-2.5 py-0.5 rounded-md text-[10px] font-bold">
                    ...
                    <!-- Pengusul & NIP -->
                    <td class="p-3">
                        <span class="font-bold text-slate-900 block">{{ $item->pengajuan->pegawai->nama ?? '-' }}</span>
                        <span class="text-[10px] text-slate-400 block font-normal">NIP/NIDN: {{ $item->pengajuan->pegawai->nip ?? '-' }}</span>
                    </td>
                    ...
                    <!-- Tombol Aksi -->
                    <td class="p-3 text-center whitespace-nowrap">
                        <a href="{{ route('admin.validasi.laporan-kemajuan.detail', $item->id) }}" class="inline-flex items-center gap-1.5 bg-[#022c22] hover:bg-emerald-900 text-white font-bold px-3.5 py-2 rounded-xl transition shadow-xs text-xs" title="Validasi Laporan">
